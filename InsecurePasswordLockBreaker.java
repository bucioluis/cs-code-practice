//UIUC CS125 FALL 2014 MP. File: InsecurePasswordLockBreaker.java, CS125 Project: Challenge7-RecursiveKnight, Version: 2015-04-20T07:56:24-0500.863101473
public class InsecurePasswordLockBreaker {
	...
	public static char[] breakLock(InsecurePasswordLock lock) {
        ...
        int enter = -1, size = 0;
        while(enter == -1)
                enter = lock.open(new char[++size]);
        return helper(lock, 0, new char[size]);
        
}
	 public static char[] helper(InsecurePasswordLock lock, int index, char[] list)
     {
             if(lock.isUnlocked()) return list;
             list[index] = 'A';
             while(lock.open(list) == index) list[index]++;
             return helper(lock, index+1, list);
     }
